...
describe('Testing -> TextInput', () => {
	...
	for (let i = 0; i < temp1.length; ++i) {
		it('To display the required div according to variant', () => {
	...
	it('To display the required div according to variant:-primary and type:-password', () => {
		render(<TextInput value='aftershoot' variant='primary' type='password' />)
		expect(screen.getByTestId('inputPrimary')).toBeInTheDocument()
		expect(screen.getByTestId('password')).toBeInTheDocument()
	})

	it('To display the required div according to variant:-primary and type:-text', () => {
		render(<TextInput value='aftershoot' variant='primary' type='text' />)
		expect(screen.getByTestId('inputPrimary')).toBeInTheDocument()
	})

	it('To display the required div according to variant:-tertiary and type:-text', () => {
